...
export class RabbitmqClientService implements OnModuleInit, OnModuleDestroy {
  ...
  // Lista de patterns permitidos
  private readonly patterns = [
    'send-email', // Enviar email al backend de intranet (usa su propio endpoint)
    // Agregar más patterns según necesites
  ];

  constructor(private configService: ConfigService) {
    this.exchangeName = this.configService.get<string>('RABBITMQ_EXCHANGE', 'pistech-exchange');
    this.queueName = this.configService.get<string>('RABBITMQ_QUEUE', 'kansaco-queue');
    ...
    this.jwtSecret = this.configService.get<string>('RABBITMQ_JWT_SECRET') || this.configService.get<string>('JWT_SECRET') || '';
    if (!this.jwtSecret) {
      this.logger.warn('RABBITMQ_JWT_SECRET o JWT_SECRET no configurado. Los mensajes RabbitMQ fallarán.');
    }
  ...
  private generateJWT(): string {
    if (!this.jwtSecret) {
      throw new Error('RABBITMQ_JWT_SECRET o JWT_SECRET no configurado. Es requerido para enviar mensajes a RabbitMQ.');
    }
    ...
    const token = jwt.sign(
      {
        sub: 'kansaco-backend', // ID del sistema para RabbitMQ
        iat: now, // Issued at (ahora - SIEMPRE actual)
        exp: now + this.jwtExpiresIn, // Expira según RABBITMQ_JWT_EXPIRES_IN
        jti: `rabbitmq-${now}-${Math.random().toString(36).substring(7)}`, // JWT ID único
      },
      this.jwtSecret,
      {
        issuer: 'kansaco-backend',
        noTimestamp: false,
      },
    );
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      this.logger.debug(
        `JWT generado: exp=${decoded.exp}, iat=${decoded.iat}, jti=${decoded.jti}, expiresIn=${this.jwtExpiresIn}s`,
      );
    }
  ...
  async sendMessage(pattern: string, data: any, origin?: string): Promise<boolean> {
    try {
      ...
      if (!this.patterns.includes(pattern)) {
        this.logger.warn(
          `Pattern no reconocido: ${pattern}. Patterns permitidos: ${this.patterns.join(', ')}`,
        );
      }
      ...
      try {
        ...
        if (timeUntilExpiry <= 0) {
          ...
          this.logger.debug(
            `✅ JWT generado para pattern ${pattern} (expira en ${timeUntilExpiry}s, emitido hace ${timeSinceIssued}s)`,
          );
        }
      } catch (e: any) {
      ...
      const messageHeaders = {
        ...
        origin: origin || this.allowedOrigins[0] || 'https://api.kansaco.com',
      };
